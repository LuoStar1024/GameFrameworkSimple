...
namespace UnityGameFramework.Runtime
{
    ...
    /// 资源加载。
    /// </summary>
    public partial class YooResourceLoadHelper : ResourceLoadHelperBase
    {
        #region Propreties
        ...
        /// 正在加载的资源列表。
        /// </summary>
        private readonly HashSet<string> m_AssetLoadings = new HashSet<string>();
        ...
        {
            // 初始化资源系统
            YooAssets.Initialize(new YooResourceLogger());
        ...
        public AssetInfo GetAssetInfo(string location, string packageName = "")
        {
            ...
            if (string.IsNullOrEmpty(packageName))
            {
                if (m_AssetInfos.TryGetValue(location, out AssetInfo assetInfo))
                {
                    return assetInfo;
                }

                assetInfo = YooAssets.GetAssetInfo(location);
                m_AssetInfos[location] = assetInfo;
                return assetInfo;
            }
            else
            {
                string key = $"{packageName}/{location}";
                if (m_AssetInfos.TryGetValue(key, out AssetInfo assetInfo))
                {
                    return assetInfo;
                }
        ...
        public override HasAssetResult HasAsset(string location, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
        ...
        /// <param name="packageName">指定资源包的名称。不传使用默认资源包。</param>
        public override async void LoadAssetAsync(string location, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
        ...
        /// <param name="userData">用户自定义数据。</param>
        /// <param name="packageName">指定资源包的名称。不传使用默认资源包。</param>
        public override async void LoadAssetAsync(string location, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
        ...
        {
            if (m_AssetLoadings.Contains(assetObjectKey))
            {
                try
                {
                    await UniTask.WaitUntil(
                            () => !m_AssetLoadings.Contains(assetObjectKey), 
